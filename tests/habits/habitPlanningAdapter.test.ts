 ...
 * itself. Nothing is re-proved here except that this adapter reaches it.
 */
...
import {
  ...
  HABIT_PRIORITY,
} from '../../lib/habits/habitPlanningAdapter.ts';
import type { HabitDefinition, HabitOccurrence, TimeWindow } from '../../lib/habits/habitTypes.ts';
...
const DAY_START = '2026-08-17T00:00:00.000Z';
const DAY_END = '2026-08-18T00:00:00.000Z';
const SCOPE = 'habits-scope';

/** 07:00–08:00 local, as the stub `TimeWindow` reads it. */
const MORNING: TimeWindow = { startMinute: 7 * 60, endMinute: 8 * 60 };
const EVENING: TimeWindow = { startMinute: 18 * 60, endMinute: 20 * 60 };

function habit(overrides: Partial<HabitDefinition> = {}): HabitDefinition {
  return {
    habitId: 'hbt_run',
    scopeId: SCOPE,
    title: 'Morning run',
...
test('an open occurrence becomes one ordinary planning item', () => {
  ...
  assert.deepEqual(sources.get('hbo_1'), { kind: 'habit_occurrence', id: 'hbo_1' });
});

test("the occurrence's own duration is used, and a bad one is reported rather than repaired", () => {
  const { items } = request([habit({ durationMinutes: 45 })], [occurrence({ durationMinutes: 20 })]);
  // The occurrence's 20, not the definition's 45: a shortened or recovered
  // occurrence is the thing being scheduled.
  assert.deepEqual(items[0].effort, { kind: 'known', minutes: 20 });

  const zero = request([habit()], [occurrence({ durationMinutes: 0 })]);
  // Passed through, so the validator can answer `EFFORT_NOT_POSITIVE` — which
  // is true of it. Substituting a default would place a block of invented
  // length and report success.
  assert.deepEqual(zero.items[0].effort, { kind: 'known', minutes: 0 });
});
...
test('the caller-supplied floor is the only lower bound the adapter sets', () => {
  const floored = buildHabitPlanningRequest({
    ...
    localDate: DATE,
    timezone: 'UTC',
    earliestStartAt: '2026-08-17T13:00:00.000Z',
  });
  assert.equal(floored.items[0].earliestStartAt, '2026-08-17T13:00:00.000Z');
  ...
  assert.equal(request([habit()], [occurrence()]).items[0].earliestStartAt, null);
});

/* ── Flexibility ─────────────────────────────────────────────────── */

test('a flexible habit produces an item with no protection key at all', () => {
  const { items } = request([habit({ flexibility: 'flexible' })], [occurrence()]);
  assert.equal(
...
test("a flexible habit still lets the user's own dragged protection carry forward", () => {
  ...
  const dragged: ScheduleBlock = {
    blockId: 'blk_1',
    scopeId: SCOPE,
    source: { kind: 'habit_occurrence', id: 'hbo_1' },
    mobility: 'flexible',
    durationMinutes: 45,
    placement: { earliestStartAt: null, latestEndAt: null, preferredWindows: [] },
    currentInterval: { startsAt: '2026-08-17T19:00:00.000Z', endsAt: '2026-08-17T19:45:00.000Z' },
    lastPlacedBy: 'user',
    lastPlanGeneration: 1,
    protection: {
      ownership: 'protected_flexible',
      origin: 'user',
      preferredInterval: { startsAt: '2026-08-17T19:00:00.000Z', endsAt: '2026-08-17T19:45:00.000Z' },
      maxShiftMinutes: 30,
    },
  };
...
test('a protected habit carries a habit_policy protection preferring its earliest window', () => {
  const { items } = request([habit({ flexibility: 'protected_flexible' })], [occurrence()]);
  assert.deepEqual(items[0].protection, {
    ownership: 'protected_flexible',
    origin: 'habit_policy',
    preferredInterval: { startsAt: '2026-08-17T07:00:00.000Z', endsAt: '2026-08-17T08:00:00.000Z' },
...
test('a protected habit with no preferred window is protected with nothing yet to retain', () => {
  const { items } = request([habit({ flexibility: 'protected_flexible', preferredWindows: [] })], [occurrence()]);
...
test('where the user kept a protected habit beats the policy window', () => {
  ...
  const { items } = request(
    ...
    [occurrence()],
    new Map([['hbo_1', '2026-08-17T19:30:00.000Z']]),
  );
  assert.deepEqual(items[0].protection?.preferredInterval, {
    ...
    endsAt: '2026-08-17T20:15:00.000Z',
  });
});
...
test('preferred windows are materialized on the day beside the items, never on them', () => {
  const { items, preferredWindows } = request(
    [habit({ flexibility: 'flexible', preferredWindows: [EVENING, MORNING] })],
    [occurrence()],
  );
  assert.deepEqual(preferredWindows.get('hbo_1'), [
    { startsAt: '2026-08-17T18:00:00.000Z', endsAt: '2026-08-17T20:00:00.000Z' },
...
test('the earliest window is the preferred placement, and a malformed one states nothing', () => {
  ...
  assert.equal(materializePreferredWindow({ startMinute: 600, endMinute: 600 }, DAY_START), null);
  assert.equal(materializePreferredWindow({ startMinute: 600, endMinute: 540 }, DAY_START), null);
  // A backwards window is not a preference, so it must not become one: the
  // habit is protected with nothing to retain rather than preferring a
  // negative-length slot.
  const backwards = habit({ flexibility: 'protected_flexible', preferredWindows: [{ startMinute: 600, endMinute: 540 }] });
  assert.equal(habitProtection(backwards, occurrence(), DAY_START, undefined)?.preferredInterval, null);
});

test('a window is measured from the day the horizon actually starts on', () => {
  // Asia/Jerusalem is UTC+3 on this date, so local 07:00 is 04:00Z. A window
  // resolved against a nominal UTC midnight would sit three hours out of step
  // with the plan it is a preference inside.
  const { items } = buildHabitPlanningRequest({
    definitions: [habit({ flexibility: 'protected_flexible' })],
    occurrences: [occurrence()],
    localDate: DATE,
    timezone: 'Asia/Jerusalem',
    earliestStartAt: null,
  });
  assert.equal(items[0].protection?.preferredInterval?.startsAt, '2026-08-17T04:00:00.000Z');
});

/* ── The three filters ───────────────────────────────────────────── */

test('an occurrence for another date is not placed in this day', () => {
  assert.deepEqual(request([habit()], [occurrence({ localDate: '2026-08-18' })]).items, []);
});

for (const state of ['completed', 'skipped', 'recovered'] as const) {
  test(`a ${state} occurrence is not offered to the planner again`, () => {
    assert.deepEqual(request([habit()], [occurrence({ state })]).items, []);
  });
}

test('a scheduled occurrence is still work and is still offered', () => {
  assert.equal(request([habit()], [occurrence({ state: 'scheduled' })]).items.length, 1);
});

for (const status of ['paused', 'archived'] as const) {
  test(`a ${status} habit's occurrences are not scheduled`, () => {
    assert.deepEqual(request([habit({ status })], [occurrence()]).items, []);
  });
}

test('an occurrence whose habit is missing has nothing to be placed as', () => {
  assert.deepEqual(request([], [occurrence()]).items, []);
});

/* ── Determinism ─────────────────────────────────────────────────── */

test('the request is a function of its content, not of the row order', () => {
  const definitions = [habit({ habitId: 'hbt_a', title: 'A' }), habit({ habitId: 'hbt_b', title: 'B' })];
  const rows = [
    occurrence({ occurrenceId: 'hbo_c', habitId: 'hbt_b' }),
    occurrence({ occurrenceId: 'hbo_a', habitId: 'hbt_a' }),
    occurrence({ occurrenceId: 'hbo_b', habitId: 'hbt_a' }),
  ];
  ...
  assert.deepEqual(forward.items.map((item) => item.itemId), ['hbo_a', 'hbo_b', 'hbo_c']);
});
...
test('the planner keeps a protected habit in its window and fills work in around it', () => {
  ...
  const habitPlacement = plan.scheduled.find((entry) => entry.itemId === 'hbo_1');
  assert.ok(habitPlacement, `the habit was not scheduled: ${JSON.stringify(plan.unscheduled)}`);
  assert.equal(habitPlacement.interval.startsAt, '2026-08-17T07:00:00.000Z');
  ...
  assert.notEqual(rivalPlacement.interval.startsAt, '2026-08-17T07:00:00.000Z');
});
...
test('a habit that cannot fit is reported, not slid into tomorrow', () => {
  ...
  // tomorrow already has its own occurrence waiting.
  const { items } = request([habit()], [occurrence({ durationMinutes: 45 })]);
  ...
  assert.equal(plan.unscheduled[0].itemId, 'hbo_1');
});
