...
test('every mobile route file exists and is enumerated', () => {
  ...
  // Forty-one with UC-3.16 (#202): `POST /api/mobile/memory/suggestions/{ruleId}`,
  // which writes a memory record from somebody's own event log.
  assert.equal(files.length, 41, `found:\n${files.join('\n')}`);
});
