...
function commitment(id: string, minutes: number, priority: ReminderPriority = 'should'): ReminderCommitment {
  return { id, startsAt: minutesFromNow(minutes), status: 'active', priority, allDay: false, postponedUntil: null };
}
  ...
  it('defers a stage inside the window to its end, and drops one too close', () => {
    withHermesIntl(() => {
      ...
      const deferred = desiredRequests(input({
        commitments: [{ id: 'c1', startsAt: new Date(morning).toISOString(), status: 'active', priority: 'should', allDay: false, postponedUntil: null }],
      ...
      const dropped = desiredRequests(input({
        commitments: [{ id: 'c2', startsAt: new Date(tooEarly).toISOString(), status: 'active', priority: 'should', allDay: false, postponedUntil: null }],
        settings: settingsWith(),
        quietHours: quiet,
        timeZone: zone,
      }));
...
describe('Must reminders and quiet hours', () => {
  it('names the reminder on its receipt by its planned instant, not where quiet hours moved it', async () => {
    ...
    const must: ReminderCommitment = {
      id: 'm1', startsAt: new Date(start).toISOString(), status: 'active', priority: 'must', allDay: false, postponedUntil: null,
    };
    const gateway = fakeGateway();
