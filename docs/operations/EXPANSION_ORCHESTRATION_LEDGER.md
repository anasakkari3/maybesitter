...
Updated: 2026-09-16
Current integration base: `ec1be12dc07d710cca20f4f72d5c9afa27b5f8b4`
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Integration | active | `program/integration-ledger-live` | `ec1be12` | this ledger; later shared config/package/privacy changes | all landed contracts | #449 and Dependabot shared files | #455 | documentation-only validation passed | open | Apple/Google store declarations later |
...
| Meeting intelligence | CI running | `program/meeting-intelligence-domain` | `ec1be12` | meeting proposal boundary; proposal tests | provider runtime; canonical commitment proposal | none | #441 | rebased focused 61 pass; typecheck and registration pass | open, head `1b6f605` | meeting-provider credentials for live verification |
| Cross-provider identity | CI running | `program/cross-provider-dedupe` | `006b2e8` | provider-independent identity decisions and adversarial integration cases | Gmail, Microsoft Graph, canonical external task | none | #459 | focused integration suite 25 pass; typecheck and registration pass; exact open-PR overlap none | open, head `2b6d817` | none |
...
| HealthKit native bridge | CI running | `program/healthkit-native-bridge` | `006b2e8` | local Expo module, iOS bridge, narrow adapters/tests | canonical readiness | shared app config remains owned by #449 | #456 | root 18 pass; mobile 167 suites / 2281 tests pass; root/mobile typecheck and registration pass; prebuild passed; full iOS simulator app build passed | open, head `03eac24` | physical iOS permission/read verification; Apple declarations |
| Health Connect native bridge | CI running | `program/health-connect-native-bridge` | `006b2e8` | local Expo module, Android bridge/manifest, narrow adapters/tests | canonical readiness | shared app config/package remains owned by #449 | #457 | root 18 pass; mobile 167 suites / 2281 tests pass; root/mobile typecheck and registration pass; prebuild passed; Health Connect Kotlin target compiles; full app assemble reaches the pre-existing Firebase notification metadata conflict in #449's app-config surface | open, head `b0af651` | Android device permission verification; Play declaration |
...
- #440 landed the aggregate RescueTime context adapter on `ec1be12`; meeting
  intelligence was immediately rebased and retested, and cross-provider
  identity opened as #459 after an exact zero-overlap check.
